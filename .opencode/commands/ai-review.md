---
description: Revisao de codigo com agentes especializados antes de abrir PR
agent: build
subtask: true
---
...
Severity follows the shared scale in the `severity-scale` skill.
Load it via the skill tool: `skill({ name: "severity-scale" })`.
Do not invent levels — each subagent reports on the same scale so findings
