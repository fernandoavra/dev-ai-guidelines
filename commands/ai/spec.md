...
argument-hint: <nome ou tema da feature/iniciativa>
...
labels you pass to AskUserQuestion MUST be in PT-BR. Internal reasoning
stays in English.

## Step 1 — Triage (one question decides the path)

Use AskUserQuestion with question:
...
Run questions ONE AT A TIME via AskUserQuestion. Wait for each answer
...
Free-form via the "Outro" option. Try to extract 1 paragraph.
...
(do not paste, just have ready if asked): "usuario consegue exportar
relatorio em PDF", "sistema rejeita CPF invalido com erro 400".
...
- One question per AskUserQuestion call. Do NOT batch all 6-10
